...
contract Cryptopuppy {
    ...
    event NewPuppy(uint id, uint parentId1, uint parentiId2);
    event PuppyTransfer(uint id, uint  salePrice, address from, address to);
    ...
    function mint(string memory _name) public payable {
        ...
        uint id = puppies.length -1;
        emit NewPuppy(id, id, id);
    }
    ...
    function buy(uint _puppyId) public payable {
        ...
        address oldOwner = puppies[_puppyId].owner;
        payable(puppies[_puppyId].owner).transfer(salePrice);
        puppies[_puppyId].owner = msg.sender;
        puppies[_puppyId].salePrice = 0;

        emit PuppyTransfer(_puppyId, salePrice, oldOwner, msg.sender);
    }
