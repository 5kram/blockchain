...
contract('Token', function(accounts) {
	...
	it('approves tokens for delegated transfer', function() {
		return Token.deployed().then(function(instance) {
			...
			return tokenInstance.allownce(accounts[0], accounts[1]);
		}).then(function(allownce) {
			assert.equal(allownce.toNumber(), 100, 'stores the allownce for delegated transfer');
		});
	});
	...
	it('handles delegated token transfers', function() {
		return Token.deployed().then(function(instance) {
			...
			return tokenInstance.allownce(fromAccount, spendingAccount);
		}).then(function(allownce) {
			assert.equal(allownce.toNumber(), 0, 'deducts the amount from the allownce');
		});
	});
